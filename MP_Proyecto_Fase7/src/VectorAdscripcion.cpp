...
istream & operator>> (istream &flujo, VectorAdscripcion &v_obj)
{
    string linea;
    Adscripcion obj;
...
//      - Comentarios, deben empezar por el caracter '#'
//      - Fin de datos, se sobreentenderá por el fin del flujo de entrada
ostream & operator<< (ostream &flujo, const VectorAdscripcion &v_obj)
{
...
void VectorAdscripcion :: GuardarVectorAdscripcion (const string & nombre) const
{
    if (!EstaVacia())
    {
        ofstream fo(nombre);
