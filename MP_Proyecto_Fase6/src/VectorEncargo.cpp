...
istream & operator>> (istream &flujo, VectorEncargo &v_obj)
{
    string linea;
    Encargo obj;
    getline(flujo, linea);
...
//      - Comentarios, deben empezar por el caracter '#'
//      - Fin de datos, se sobreentenderá por el fin del flujo de entrada
ostream & operator<< (ostream &flujo, const VectorEncargo &v_obj)
{
...
void VectorEncargo :: GuardarVectorEncargo (const string & nombre) const
{
    if (!EstaVacia())
    {
        ofstream fo(nombre);
...
//vacío.
void VectorEncargo :: RecuperarVectorEncargo (const string & nombre)
{
    ifstream fi(nombre);
...
int operator&& (const VectorEncargo &v_obj,\
                        const string &numero)
{
    return v_obj.BuscarEncargo(stoi(numero));
}
...
// contiene al dato Encargo cuyo campo clave coincide con el string.
// Si no está contenido, devuelve 0.
int operator&& (const string &numero,\
                        const VectorEncargo &v_obj)
{
    return v_obj && numero;
}
...
/***********************************************************************/
// VALOR: Devuelve el valor de la Encargo en la posición "indice"
